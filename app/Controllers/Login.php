				...
				if(empty($_POST['txtEmail']) || empty($_POST['txtPassword'])){
					...
					if(empty($requestUser)){
						...
						if($arrData['status'] == 1){
							...
							$_SESSION['rutUser'] = $arrData['identificacion'];
							...
							$arrResponse = array('status' => true, 'msg' => 'ok','rolid' => $arrData['rolid']);
						}else{
			...
			if($_POST){
				...
				if(empty($_POST['txtEmailReset'])){
					...
					if(empty($arrData)){
						...
						$url_recovery = 'https://www.respaldoschile.cl/login/confirmUser/'.$strEmail.'/'.$token;
						...
						if($requestUpdate){
							...
							if($sendEmail){
								...
								$arrResponse = array('status' => false, 
												 'msg' => 'No es posible realizar el proceso, intenta más tardes.' );
							}
						}else{
							$arrResponse = array('status' => false, 
