		...
		if($_SESSION['permisosMod']['r']){
			...
			for ($i=0; $i < count($arrData); $i++) {
				...
				if($arrData[$i]['idtransaccionpaypal'] != ""){
					$arrData[$i]['transaccion'] = $arrData[$i]['datospaypal'];
				}
				...
				if($_SESSION['permisosMod']['r']){
					...
					if($arrData[$i]['idtipopago'] == 1){
						$btnView .= '<a title="Ver Transacción" href="'.base_url().'/pedidos/transaccion/?token_ws='.$arrData[$i]['datospaypal'].'" target="_blanck" class="btn btn-info btn-sm"> <i class="fa fa-paypal" aria-hidden="true"></i> </a> ';
					}else{
