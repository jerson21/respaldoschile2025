...
<div class="bg0 m-t-23 p-b-140">
	<div class="container">
		<span class="stext-102" style="background:black;color:white;padding:5px;border-radius: 5px; "><i
				class="fa-solid fa-cart-shopping"></i> Productos en Oferta</span>
		...
		<div class="text-center"><span class="stext-107"
				style="background:black;color:white;padding:2px;border-radius: 5px; ">&#8226; Para elegir tamaños y
				colores ingresa al producto &#8226;</span></div>
		...
		<div id="landing_home1" class="autotag">
			...
			<section id="lo_tenemos_todo">
				<div id="carrusel_tenemos_todo">
					...
					<div id="splide_1" class="splide splide--loop splide--ltr splide--draggable is-active"
						style="visibility: visible;">
						<div class="splide__arrows"><button class="splide__arrow splide__arrow--prev" type="button"
								aria-controls="splide_1-track" aria-label="Previous slide"><svg
								...
								aria-controls="splide_1-track" aria-label="Next slide"><svg
									xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40" width="40" height="40">
									<path
										d="m15.5 0.932-4.3 4.38 14.5 14.6-14.5 14.5 4.3 4.4 14.6-14.6 4.4-4.3-4.4-4.4-14.6-14.6z">
									</path>
								...
								<?php if (count($data['categorias']) > 0) {
									foreach ($data['categorias'] as $categoria) {
										...
										<div class="tenemos_todo1">
											...
											<a href="<?= base_url() ?>/tienda/categoria/<?= $categoria['idcategoria'] . '/' . $categoria['ruta'] ?>"
												rpl_sp="carrusel-cat-ltt-_-dormitorio-_-https://simple.ripley.cl/dormitorio">
												<div class="circulo">
													<picture>

														<source media="(min-width: 700px)" type="image/webp"
															srcset="<?= $categoria['portada'] ?>" alt="">

														<source media="(min-width: 700px)"
															srcset="<?= $categoria['portada'] ?>">
		...
		<div class="row isotope-grid">
			...
			if (count($arrProductos) > 0) {
					...
					<div class="col-sm-6 col-md-4 col-lg-3 p-b-35 isotope-item women">
						...
						<div class="block2">
							...
							<div class="block2-pic hov-img0" style="border-radius: 30px; z-index: 0;">
								<?php if ($categoria == 8 && $descuento > 0) { ?> <span class="oferta">-
										<?= round($descuento_actual); ?>%</span> <?php } ?>
								...
								<?php //if($arrProductos[$p]['tipo_prod'] == "1"){
										...
										if (isset($arrProductos[$p]['descuento']) && is_array($arrProductos[$p]['descuento']) && count($arrProductos[$p]['descuento']) > 0) {
											...
											if ($precio > 0) {
												...
												$descuento = round($multiplic / $precio);
						...
						<?php if ($arrProductos[$p]['categoria'] != 'Colchones') { ?>
							...
							<span class="stext-107"
								style="background:#C6C6C6;color:white;padding:2px;border-radius: 5px; padding-left: 5px;padding-right: 5px;">
								Color Unico</span>
						<?php } ?>
				...
				?>
				<p>No hay productos para mostrar <a href="<?= base_url() ?>/promociones"> Ver productos</a></p>
			<?php } ?>
		...
		if (count($data['productos']) > 0) {
			...
			<div class="flex-c-m flex-w w-full p-t-45">
				<?php if ($data['pagina'] > 1) { ?>
					<a href="<?= base_url() ?>/promociones/page/<?= $prevPagina ?>"
				...
				<?php if ($data['pagina'] != $data['total_paginas']) { ?>
					<a href="<?= base_url() ?>/promociones/page/<?= $nextPagina ?>"
