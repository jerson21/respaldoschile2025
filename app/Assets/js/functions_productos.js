...
// Inicialización de DataTable para visualizar productos.
// Configura la obtención de datos desde el servidor y define las columnas.
...
tableProductos = $('#tableProductos').dataTable( {
    ...
    "ajax":{
        ...
        // Definición de columnas (id, código, nombre, stock, precio, estado, opciones).
...
window.addEventListener('load', function() {
    if(document.querySelector("#formProductos")){
        ...
        // Realiza validaciones y envía datos a un endpoint del servidor.
        ...
        // Prevenir el comportamiento por defecto del formulario.
            ...
            let precioUnico = document.querySelector('#precio_unico').checked;
            ...
            if(strNombre == '' || intCodigo == '' || strPrecio == '' || intStock == '')
            {
              ...
              // Verifica si se requiere enviar los materiales
              if (!precioUnico) {
                  let checkboxes = document.querySelectorAll('#checkMaterial:checked');
                  if (checkboxes.length === 0) {
                      swal("Atención", "Debe seleccionar al menos un material.", "error");
                      return false;
                  }
              }
...
// Función para mostrar información detallada del producto en un modal.
...
// Función para cargar datos en el formulario para editar un producto.
...
function fntEditInfo(element,idProducto){
    ...
    request.onreadystatechange = function(){
      ...
      var categoria = objProducto.categoriaid;   
        precios_sofas.style.display = "none";
            producto_colores .style.display = "none";
            precios_banqueta .style.display = "none";
             precios_velador.style.display = "none";
...
//////////////////AGREGADO RECIEN.
     var precio_unicoa = $('input:checkbox[name=precio_unico]:checked').val();
            ...
            if (objProducto.tamanos && objProducto.tamanos.length > 0) {
      let tamanos = objProducto.tamanos;
    for (let t = 0; t < tamanos.length; t++) {
        let tamano = tamanos[t].tamano.replace(/\s+/g, '').toLowerCase();
        
        // Reemplazar '1 1/2' por 'plazaymedia' si es necesario
        if (tamano === '11/2') {
            tamano = "plazaymedia";
        }
        ...
        let precio = tamanos[t].precio;
        
        // Buscar los inputs por su nombre
        let inputElement = document.querySelector(`[name="${tamano}"]`);
        
        if (inputElement) {
            // Establecer el valor solo si se encontró un campo coincidente
...
// Funciones para cargar dinámicamente las opciones de categorías y colores.
...
// Funciones para abrir modales para la creación de nuevos productos.
...
function openModal()
{
    ...
    document.querySelector('#titleModal').innerHTML = "Nuevo Productos";
