...
switch ($opcion) {
  ...
  case 1:
    $externo = false;
    $resultado = $mysqli->query("SELECT c.*, d.*
                            FROM clientes c
  ...
  case 2:
    ...
    $resultado = $mysqli->query("SELECT * FROM pedido p
  INNER JOIN pedido_detalle pd ON p.num_orden = pd.num_orden where rut_cliente ='$id' and estadopedido <3");
