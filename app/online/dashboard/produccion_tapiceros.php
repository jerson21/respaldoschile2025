...
<div id="contenido1" style="margin:0 auto; text-align: center;">
    <div
        style="background: white; border:solid thin; border-radius: 15px; border-color: #D1D1D1; width:700px; height: 100%; display: inline-block; margin-top: 50px; padding: 5px;">
        <h1>Tapicero 1</h1>


        <table id="tablatapiceross" class="table table-striped table-bordered table-condensed"
            style="width:100%; font-size:0.8rem; padding: 5px; ">
            ...
            $resultados = $mysqli->query("SELECT d.*,pd.*
...
ORDER BY d.estadopedido DESC,d.tamano ASC, d.color ASC");
            ...
            while ($dat = mysqli_fetch_array($resultados)) {
                ...
                if ($modeloa == 'Botone') {
                    $modeloa = "botoné Madrid, ";
                }
                if ($modeloa == 'Botone 4 corridas de botones') {
                    $modeloa = "botoné 4 corridas de botones, ";
                }
                if ($modeloa == 'Botone 3 corridas de botones') {
                    $modeloa = "botoné 3 corridas de botones, ";
                }
                ...
                if ($plazas == "Seleccionar las plazas") {
                    $plazas = ".Consultar tamaño.";
                    $base = "";
                }
                if ($tipo_boton == "B D") {
                    $tipo_boton = ", Agregar botón Diamante";
                }
                if ($anclaje == "si") {
                    $anclaje = ", Ocupar esqueleto con Madera de anclaje";
                }
                if ($anclaje == "patas") {
                    $anclaje = ", Ocupar esqueleto con Patas de madera";
                }
                if ($anclaje == "no") {
                    $anclaje = "";
                }
                if ($dat['alturabase'] == "1.90mt") {
                    $base = " , Largo uno noventa";
                }
                if ($dat['alturabase'] == "2Mt") {
                    $base = " , Largo dos metros";
                }
                if ($dat['comentarios'] != "") {
                    $comentario = ". Leer pantalla para ver detalles.";
                }
                ...
                if ($dat['modelo'] == "Liso con costuras") {
                    $modeloa = "Liso Costuras Venecia, ";
                }
                if ($dat['modelo'] == "Capitone Con Orejas y Tachas") {
                    $modeloa = "Capitoné Tokio Con Orejas y Tachas, ";
                }
                if ($dat['modelo'] == " Capitone Con Orejas") {
                    $modeloa = "Capitoné Tokio Con Orejas, ";
                }
                if ($color == 'CAFE') {
                    $color = ", café ";
                }
                if ($dat['color'] == 'AZUL PETROLEO') {
                    $color = ", AZUL PETRÓLEO";
                }

                if ($modeloa == "Botone Madrid") {
                    $modeloa = "Botoné Madrid";
                }
                ...
                if ($cortada != '') { ?>
                    ...
                    <td style="height:10px; padding: 1px; "><?php echo $dat['id'];
                    if ($dat['confirma'] == "1") {
                        echo $boton = ' <button type="button" class="btn btn-success btn-circle btn-sm" style="width: 5px; height: 5px; padding: 0; border-radius: 50%;" ></button>';
                    } else {
                        echo $boton = ' <button type="button" class="btn btn-danger btn-circle btn-sm" style="width: 5px; height: 5px; padding: 0; border-radius: 50%;" ></button>';
                    } ?></td>
                    ...
                    <td style="height:10px; padding: 1px; text-align: center;">
                        ...
                        if ($dat['estadopedido'] == "2") { // si el estado pedido esta en 2 osea enviado a fabricacion mostrar el 5 que es el color de "fabricando"
                            ...
                            $boton = ' <button type="button" class="btn btn-warning btn-circle btn-sm btnpedidolistojaime" data-value="' . $detalle . '"  data-estado="2"></button>';

                        }
                        if ($dat['estadopedido'] == "5") {

                            $boton = ' <button type="button" class="btn btn-info btn-circle btn-sm btnpedidolistojaime" data-value="' . $detalle . '" data-estado="5"></button>';

                        }
                        if ($dat['estadopedido'] == "6") {
                            $boton = ' <button type="button" class="btn btn-success btn-circle btn-sm btnpedidolistojaime" data-value="' . $detalle . '" data-estado="6" ></button>';
                        }
...
<i class="fas fa-truck"></i>';
    ...
    <div
        ...
        <table id="tablatapiceross" class=" table-striped table-bordered table-condensed"
            style="width:100%; font-size:0.8rem; padding: 5px; ">
            <thead class="text-center">
                <tr>
...
ORDER BY d.estadopedido DESC,d.tamano ASC, d.color ASC");
            ...
            while ($dat = mysqli_fetch_array($resultados)) {
                ...
                // $cortada = ""; //COMENTAR SI NOQUEREMOS QUE APAREZCA EL MENSAJE DE QUE LA TELA NO ESTA LISTA
            
                $proceso3Encontrado = false;
                $idPedido = $dat['idPedido'];

                $consultaProceso = "SELECT COUNT(*) AS existeProceso FROM pedido_etapas WHERE idPedido = $idPedido AND idproceso = 3";
                $resultadoProceso = $mysqli->query($consultaProceso);
                $filaProceso = mysqli_fetch_assoc($resultadoProceso);

                if ($filaProceso['existeProceso'] > 0) {
                    $telalista = 1;
                    // El proceso 3 existe para este pedido.
                    // Aquí puedes hacer lo que necesites basado en esta información.
                } else {
                    $telalista = 0;
                    $cortada = "La tela aún no está lista. ";
                ...
                if ($telalista == '0') { ?>
                    ...
                    <tr style="background-color: #FFF2F2 !important; ">
                    <?php } else { ?>
                    <tr>
                    <?php } ?>
                    ...
                    <td style="height:10px; padding: 1px; "><?php echo $dat['id'];
                    // CONSULTAR SI EL CLIENTE CONFIRMO LA ENTREGA.
                    if ($dat['confirma'] == "1") {
                        echo $boton = ' <button type="button" class="btn btn-success btn-circle btn-sm" style="width: 5px; height: 5px; padding: 0; border-radius: 50%;" ></button>';
                    } else {
                        echo $boton = ' <button type="button" class="btn btn-danger btn-circle btn-sm" style="width: 5px; height: 5px; padding: 0; border-radius: 50%;" ></button>';
