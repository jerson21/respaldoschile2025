$(document).ready(function(){
    ...
    "language": {
       ...
       "columnDefs":[{
        "targets": -1,
        "data":null,
        "defaultContent": "<div style=' fon-size:0.8rem;'><div class='btn-group'><button class='btn btn-primary btnEditar' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Editar</button><button class='btn btn-danger btnBorrar' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;''>Borrar</button></div></div>"  
       }],
...
var fila; //capturar la fila para editar o borrar el registro
    $(document).on("click", ".btnEditar", function(){
        ...
        inputCampos.forEach(function(input) {
         ...
         // Asignar los valores recibidos a los campos correspondientes
                    ...
                    if(response.estadopedido === "0"){   $("#estadopedido").val("En Espera");  }
                    if(response.estadopedido === "1"){   $("#estadopedido").val("Aceptado");  }
                    if(response.estadopedido === "2"){   $("#estadopedido").val("Enviado a fabricacion");  }
            
                    if(response.estadopedido === "5"){ Swal.fire({
          title: 'Pedido en fabricación',  text: 'El pedido se encuentra actualmente en proceso de fabricación. No se pueden realizar modificaciones en este momento.',  icon: 'info',
          confirmButtonText: 'Aceptar',   customClass: {    confirmButton: 'btn btn-primary'  }
        });
          $("#estadopedido").val("Fabricando");  }
                    ...
                    if(response.estadopedido === "6"){
                        Swal.fire({
          title: 'Pedido Listo',  text: 'El pedido esta listo. No se pueden realizar modificaciones.',  icon: 'info',
          confirmButtonText: 'Aceptar',   customClass: {    confirmButton: 'btn btn-primary'  }
        });
            ...
            $("#detalles_fabricacion").val(response.detalles_fabricacion).prop("disabled", false);
           ...
           $("#fecha_retiro").css("display", "block");
            ...
            $("#tipo_entrega").val(response.metodo_entrega).prop("disabled", false);
  ...
  window.open(url, '_blank');
        ...
        for (var i = 0; i < etapas.length; i++) {
          ...
          label.innerText = etapas[i].NombreProceso + ": " + etapas[i].fecha + " " + etapas[i].usuario+" "+etapas[i].obs;
...
$(document).on("click", ".btnEditarOrden", function(){
        ...
        inputCampos.forEach(function(input) {
          input.value = "";
            ...
            id = fila.find('td:eq(1)').text();
             cant = fila.find('td:eq(0)').text();
            // cargarPagosAsociados(id);

            
            // Realizar una consulta a la base de datos utilizando el ID
            $.ajax({
                url: "get_pedido.php?method=num_orden",
                type: "post",
                data: {id: id},
                dataType: "json",
                success: function(resultado) {
                  ...
                  var total_pagadoa = resultado.total_pagado; 
          
                  // Si necesitas trabajar con un pedido específico
        ...
        // Iterar sobre cada pedido en el objeto de resultados
        Object.values(pedidos).forEach(function(pedido) {
          const capitalize = (t) => { let lowerCaseText = t.toLowerCase();   return lowerCaseText.charAt(0).toUpperCase() + lowerCaseText.slice(1); }

            // Aquí asumimos que cada pedido tiene una estructura como la que mostraste
            // Agrega una fila a la tabla por cada pedido
            tbody.append(
                `<tr>
                    <td>${pedido.ide}</td>
                    <td>${pedido.modelo} ${pedido.plazas} ${capitalize(pedido.tela)} ${capitalize(pedido.color)}</td>
                    <td>1</td> <!-- Asumiendo que no hay un campo específico para cantidad -->
                    <td>${pedido.direccion} ${pedido.numero} / ${pedido.dpto}, ${pedido.comuna}</td>
                    <td>${pedido.precio}</td>
                    <td>N/A</td> <!-- No veo una 'Referencia de Pago' en tu estructura de datos -->
                    <td>${pedido.fecha_ingreso}</td>
                    <td>${pedido.estadopedido}</td> <!-- Asegúrate de mapear este estado a un texto si es necesario -->
                </tr>`
            );

            // Si necesitas incluir detalles de las etapas, podrías agregarlos aquí.
            // Nota: Dependiendo de cómo desees mostrar las etapas, es posible que necesites
            // estructurar los datos de manera diferente o incluso modificar la tabla para acomodarlos.
        });



         // Asignar los valores recibidos a los campos correspondientes
                    $("#n_orden").val(response.num_orden);
                    $("#rutb").val(response.rut);
                    $("#nombreb").val(response.nombre);
                    $("#ide_webs").show();
                     $("#ide_web").val(response.ide_webs);
                     $("#costo_envio").val(response.costo_envio);
                     $("#totalpagado").val(total_pagadoa);
                    ...
                    if(response.estado_orden === "0"){   $("#estadopedido").val("En Proceso");  }
                    if(response.estado_orden === "1"){   $("#estadopedido").val("Finalizado");  }
                    if(response.estado_orden === "2"){   $("#estadopedido").val("Enviado a fabricacion");  }
            
                    if(response.estado === "5"){ Swal.fire({
          title: 'Pedido en fabricación',  text: 'El pedido se encuentra actualmente en proceso de fabricación. No se pueden realizar modificaciones en este momento.',  icon: 'info',
          confirmButtonText: 'Aceptar',   customClass: {    confirmButton: 'btn btn-primary'  }
        });
          $("#estadopedido").val("Fabricando");  }
          ...
          confirmButtonText: 'Aceptar',   customClass: {    confirmButton: 'btn btn-primary'  }
            ...
            $("#anclaje").val(response.anclaje).prop("disabled", true);
            $("#tipo_boton").val(response.tipo_boton).prop("disabled", true);
            $("#tela").val(response.tela.toUpperCase()).prop("disabled", true);
           ...
           $("#fecha_retiro").css("display", "block");
            ...
            $("#telefono").val(response.telefono).prop("disabled", false);
            $("#precio").val(totalPrecioTodosLosPedidos).prop("disabled", false);
            $("#abono").val(response.abono).prop("disabled", false);
            $("#comentarios").val(response.comentarios).prop("disabled", false);
            $("#tipo_entrega").val(response.metodo_entrega).prop("disabled", false);
       ...
       id = parseInt($(this).closest("tr").find('td:eq(0)').text());
        tamano = $(this).closest("tr").find('td:eq(1)').text();
         ...
         color = $(this).closest("tr").find('td:eq(5)').text();
          ...
          telaycolor = $(this).closest("tr").find('td:eq(4)').text()+" "+$(this).closest("tr").find('td:eq(5)').text();
              ...
              Swal.fire({
                ...
                if (result.isConfirmed) {
                  Swal.fire({
                    ...
                    html: telaycolor + '<br>' + metraje + ' Mt o ' + aldobles + ' al dobles<hr>' +
                      'Razón de corte:<br>' +
                    ...
                    preConfirm: () => {
                      const metrajeAdicional_ingresado = Swal.getPopup().querySelector('#swal2-input').value;
                      const metrajeAdicional = convertirCmAMetros(metrajeAdicional_ingresado);
                      const checkboxes = Swal.getPopup().querySelectorAll('input[name="razon"]:checked');
                      const razonesCorte = Array.from(checkboxes).map(checkbox => checkbox.value);
                      let otraRazon = '';
                      if (razonesCorte.length === 0) {
                        Swal.showValidationMessage('Debes seleccionar al menos una razón de corte');
                        return false;
                      }
                    ...
                    if (result.isConfirmed) {
                      const { metrajeAdicional, razonesCorte, otraRazon } = result.value;
                      // Verificar si se han completado los campos antes de enviar la solicitud AJAX
                      if (metrajeAdicional && (razonesCorte.length > 0 || otraRazon)) {
                        // Realizar la solicitud AJAX o la lógica adecuada para enviar el formulario
                        
                        enviarSolicitudAjax(metrajeAdicional, razonesCorte, otraRazon);
                      } else {
                  ...
                  const otraRazonCheckbox = Swal.getPopup().querySelector('input[name="razon"][value="Otro"]');
                  const otraRazonInput = Swal.getPopup().querySelector('#otraRazon');
                  otraRazonCheckbox.addEventListener('change', (event) => {
                    otraRazonInput.style.display = event.target.checked ? 'block' : 'none';
                  });
            ...
            $.ajax({
                ...
                success: function(data){
                    if (data == 1) {
                        ...
                        Swal.fire("Error", "Ocurrió un error al asignar el respaldo", "error").then(() => {
                            location.reload(true);
                        });
                    }
                },
...
var respuesta = confirm("¿Está seguro de Agregar a Tapicero 3 el respaldo: "+id+"?");
    if(respuesta){
        $.ajax({
...
console.log(detalle);
   ...
   if(estadoactual == 5){
    ...
    function seleccionarVozHombre() {
      ...
      const vozDeHombre = vocesDisponibles.find(voice => voice.name === 'Google español');
...
.then((result) => {
  if (result.isConfirmed) {
    $.ajax({
      ...
      type: "POST",
      dataType: "json",
      data: { opcion: 16, id: id, id_tapicero: '1', estadopedido: estadopedido },
      success: function(data) {
        if (data === 1) {
...
.then((result) => {
  if (result.isConfirmed) {
    $.ajax({
      ...
      type: "POST",
      dataType: "json",
      data: { opcion: 16, id: id, id_tapicero: '2', estadopedido: estadopedido },
      success: function(data) {
        if (data === 1) {
          swal.fire("Asignado correctamente", "", "success");
        } else {
...
var respuesta = confirm("¿Tapicero 3 Confirma termino de respaldo : "+id+"?");
    if(respuesta){
        $.ajax({
    ...
    $.ajax({
        ...
        success: function(data){  
            ...
            if(estadoped == 1){
                                ...
                                if(estadoped == 19){
                                    estadoped = "<div class='text-center' ><div class='btn-group'><button class='btn btn-warning btnEditarestado' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Reagendar</button></div>";
                                }
        ...
        success: function(data){  
            ...
            numero = data[0].numero;
             ...
             if(data[0].cod_ped_anterior == ""){
                ...
                reemitir = "<a href='cambio_prod.php?id="+id+"'><button class='btn btn-danger' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Re-Emitir</button></a>";
             }
            ...
            if(data[0].estadopedido == "1"){
                                ...
                                if(data[0].estadopedido == 2){
                                   estadoped = "<div class='text-center' ><div class='btn-group'><button class='btn btn-warning btnEditarestado' id='parpadea_' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Por Fabricar</button></div>";
            ...
            if(opcion == 1){tablaPersonas.row.add([id,nombre,pais,edad]).draw();}
                 ...
                 tablapedidos.row(fila).data([cant,id,rut_cliente,modelo,plazas,alturabase,material,color,direccion,numero,comuna,telefono,precio,reemitir,estadoped,,]).draw();}             
        },
        error:function(){
                   alert("Error cambiando estado");
    ...
    $.ajax({
        ...
        success: function(data){  
            ...
            if(data[0].formadepago == "15"){
                                   estadoped = "<div class='text-center' ><div class='btn-group'><button class='btn btn-success btnEditarestado' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Efectivo</button></div>";
                                }if(data[0].formadepago == 16){
                                   estadoped = "<div class='text-center' ><div class='btn-group'><button class='btn btn-success btnEditarestado'  style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Debito</button></div>";
                                }if(data[0].formadepago == 17){ 
                                    estadoped = "<div class='text-center' ><div class='btn-group'><button class='btn btn-success btnEditarestado' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Credito</button></div>";
                                }
                                if(data[0].formadepago == 18){
                                    estadoped = "<div class='text-center' ><div class='btn-group'><button class='btn btn-success btnEditarestado' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Transferencia</button></div>";
                                }
                                if(data[0].formadepago == 19){
                                    estadoped = "<div class='text-center' ><div class='btn-group'><button class='btn btn-success btnEditarestado' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Webpay</button></div>";
                                }
                                if(data[0].formadepago == ""){
                                    estadoped = "<div class='text-center' ><div class='btn-group'><button class='btn btn-warning btnEditarestado' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Por Pagar</button></div>";
                                }
                                ...
                                if(data[0].pagado == 1){
                                    pagado = "<div class='text-center' ><div class='btn-group'><button class='btn btn-primary' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Pagado</button></div>";
                                }
                                
            if(opcion == 1){tablavalidarpagos.row.add([id,nombre,pais,edad]).draw();}
            else{
                tablavalidarpagos.row(fila).data([id,rut_cliente,nombre,modelo,plazas,alturabase,color,direccion,telefono,modopago,precio,pagado,estadoped,,]).draw();
                tablavalidarpagos.row.add.data([id,rut_cliente,nombre,modelo,plazas,alturabase,color,direccion,telefono,modopago,precio,pagado,estadoped,,]).draw();}             
        },
        error:function(){
...
$("#editarpedido").submit(function(e){
    ...
    $.ajax({
        ...
        type: "POST",
        dataType: "json",
        data: {ide:ide,rut:rut,nombre:nombre,telefono:telefono,tipo_boton:tipo_boton,detalle_entrega:detalle_entrega,metodo_entrega:metodo_entrega,anclaje:anclaje,detalles_fabricacion:detalles_fabricacion, opcion:5,modelo:modelo,plazas:plazas,tela:tela,color:color,alturabase:alturabase,direccion:direccion,numero:numero,comuna:comuna,precio:precio,abono:abono,comentarios:comentarios},
        success: function(data){  
            console.log(data);  
            ...
            if(data[0].estadopedido == "1"){
                                   ...
                                   estadoped = "<div class='text-center' ><div class='btn-group'><button class='btn btn-warning btnEditarestado' id='parpadea' style='font-size:0.8rem;max-height:1.5rem; line-height:12px;'>Por Fabricar</button></div>";
                                }
            ...
            if(opcion == 1){tablapedidos.row.add([id,nombre,pais,edad]).draw();}
            else{
                tablapedidos.row(fila).data([cant,id,rut_cliente,modelo,plazas,alturabase,material,color,direccion,numero,comuna,telefono,precio,reemitir,estadoped,,]).draw();
                }            
        },
...
$("#editarpedido_general").submit(function(e){ //funcion para editar pedido del boton todos los pedidos
    e.preventDefault();    
    ide = $("#ide").val();
    ...
    $.ajax({
        ...
        type: "POST",
        dataType: "json",
        data: {ide:ide,rut:rut,telefono:telefono, opcion:6,modelo:modelo,plazas:plazas,tela:tela,color:color,alturabase:alturabase,direccion:direccion,numero:numero,comuna:comuna},
        success: function(data){  
            console.log(data);  
            ...
            if(opcion == 1){tablaPersonas.row.add([id,nombre,pais,edad]).draw();}
                ...
                tablaPersonas.row(fila).data([id,rut_cliente,modelo,plazas,alturabase,material,color,direccion,numero,comuna,telefono,,estadoped,precio,,]).draw();
                }            
        },
        error:function(){
                   alert("Error cambiando estado");
